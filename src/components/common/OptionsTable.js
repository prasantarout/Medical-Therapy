...
const OptionsTable = (props) => {
    ...
    const [isCheckedArray, setIsCheckedArray] = useState(feedBackData.map(() => false));
    ...
    const renderfeedbackItem = ({ item, index }) => {
        ...
        return (
            <View style={[styles.tableRow, css.row, css.w100, { backgroundColor: bgRow }]}>
                <View style={[css.f1, css.px3, styles.titleWrap, styles.tableCellInner]}>
                    <Txt style={[css.fs18, css.semiBold]} >{item.title}</Txt>
                </View>
                <View style={[styles.tableCell, styles.tableCellInner, styles.notSatisfied]}>
                    <Checkbox onPress={() => updateData(item, index, 'notsatisfied')} isChecked={isCheckedArray[index]?.notsatisfied} />
                </View>
                <View style={[styles.tableCell, styles.tableCellInner, styles.needImprovement]}>
                    <Checkbox onPress={() => updateData(item, index, 'needimprovement')} isChecked={isCheckedArray[index]?.needimprovement} />
                </View>
                <View style={[styles.tableCell, styles.tableCellInner, styles.satisfied]}>
                    <Checkbox onPress={() => updateData(item, index, 'satisfied')} isChecked={isCheckedArray[index]?.satisfied} />
                </View>
                <View style={[styles.tableCell, styles.tableCellInner, styles.great]}>
                    <Checkbox onPress={() => updateData(item, index, 'great')} isChecked={isCheckedArray[index]?.great} />
                </View>
            </View>
        )
    }
